...
const Controls = styled.div` display: flex; gap: 12px; margin-bottom: 14px; `;
...
export default function Workflows() {
  const { token, userId } = useAuth(); // userId is the DB user ID
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
  const [q, setQ] = useState("");

  // Load ONLY this user's docs from your Node API (server-side filter).
  // Fallback: we'll still client-filter below in case the API ignores the param.
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        ...
        setErr("");
        if (userId == null) {
          // If user not ready yet, don't fetch everything then filter — wait.
          return;
        }
  ...
  // First filter by ownership (client-side safety),
  // then apply the search box filter.
  ...
  // Download via .NET proxy (kept as-is)
    ...
    <Page>
      ...
      {!loading && !err && userId != null && (
        <Table>
          ...
          <tbody>
            {filtered.map((r, i) => (
              <tr key={r._id || r.id || r.signNowDocumentId}>
                <td>{r.id ?? i + 1}</td>
                ...
                <td><Small>{r.signNowDocumentId}</Small></td>
                <td>
